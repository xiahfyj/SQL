...
from (select custid,count(distinct loanid) loan_count from all_loans where payoffdate is  null and writeoffdate is  null 
...
up principal+paid for each loan and each customer if they took place before 6 months after first date being a customer, call
...
number of loan missed/total number of loans. And I assumed that if someone missed a payment then amount_paid for that 
week is null.
First, get the min date after approvedate as first payment date.Join this table with two existing tables,making sure we only kept the ones has null value for amount_paid on the 
...
select temp1.custid, temp1.total_paid/temp2.total_amount as profitability from(select a.custid, sum(b.amount_paid)as total_paid from all_loans a left join all_loanhist b 
...
select temp3.custid from(select temp1.custid, dense_rank()over(order by temp1.total_paid/temp2.total_amount)as rnk from(select a.custid, sum(b.amount_paid)as total_paid from all_loans a left join all_loanhist b 
on a.loanid=b.loanid  group by a.custid) temp1,
(select custid, sum(amount) as total_amount from all_loans  group by custid)temp2
